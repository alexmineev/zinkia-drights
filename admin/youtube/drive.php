...
if (!isset($_REQUEST['langs'])) $_REQUEST['langs'] = null;
...
function genCSVFile($db,$year,$trimester,$cms,$channels) {
 ...
 if (isset ($_REQUEST['episode']) && $_REQUEST['episode']>0) {
    ...
    if (!is_null($_REQUEST['langs'])) {
        ...
        $filterSQL .= " AND lang_id IN ($langs)";
      
     }
    if ($trimester!=0)
      $res = $db->query("SELECT id,title,channel,serie,lang_id, season,episode,views, earnings,thumbnail FROM videos WHERE $filterSQL ORDER BY channel,serie");
    else 
      $res = $db->query("SELECT id,title,channel,serie,season,episode,lang_id sum(views) as views, sum(earnings) as earnings,thumbnail FROM videos WHERE $filterSQL GROUP BY id ORDER BY channel,serie");
...
ORDER BY channel"
    ...
    {
        $csvLines[] = array("Thumbnail","Canal","Serie","Titulo","Idioma","Temporada/Episodio","Reproduciones","Ganancias");

...
class GlassCube {
    ...
    public function fetchComments($taskUri) {
        echo ">>>>> Fetching comments from ".$taskUri."\n";
    ...
    public function genReport() {
        ...
        $pageTokens = explode("-",$pages[0]);
        
        for ($i=1;$i<55;$i++) {
            $pageTokens[9]=$i;
            $this->pages[] = implode("-",$pageTokens);
        }
...
//$sections = $gc->getSectionList();
   ...
   for ($j=0;$j<count($tasks);$j++) {
        $tasks[$j]['comments'] = $gc->fetchComments($tasks[$j]['id']);
   }
